...
<div class="max-w-xl mx-auto px-4 py-6">
    ...
    <div class="bg-white/90 backdrop-blur shadow-lg rounded-2xl p-6 border border-slate-200">
        ...
        <div class="flex items-center gap-3 mb-6">
            <div class="flex items-center justify-center h-12 w-12 rounded-xl bg-amber-100 text-amber-600">
                <i class='bx bx-edit text-3xl'></i>
            </div>

            <div>
                <h2 class="text-2xl font-semibold text-slate-800">Editar Concepto</h2>
        ...
        <!-- FORMULARIO -->
        <form action="{{ route('conceptos.update', $concepto) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center gap-3 pt-2">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox"
                           name="es_becable"
                           value="1"
                           {{ old('es_becable', $concepto->es_becable) ? 'checked' : '' }}
                           class="h-5 w-5 rounded-md border-slate-300 text-amber-600 focus:ring-amber-500">
            ...
            <div class="flex justify-end gap-2 pt-4">
                ...
                <button type="submit"
                        class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500 text-white
                               font-medium shadow-md hover:bg-amber-600 transition">
