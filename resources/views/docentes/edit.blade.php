...
<div class="max-w-4xl mx-auto mt-6">
    <div class="bg-white/90 backdrop-blur shadow-lg rounded-2xl p-6 border border-slate-200">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-semibold text-slate-800 flex items-center gap-2"><i class='bx bx-edit-alt text-3xl text-amber-600'></i> Editar Docente</h1>
                <p class="text-xs text-slate-500 mt-1">Modifica datos académicos, fiscales y bancarios necesarios para pago docente.</p>
            </div>
        ...
        @endif
        <form method="POST" action="{{ route('docentes.update', $docente) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" class="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-medium shadow-md transition">Actualizar Docente</button>
            </div>
        </form>
    </div>
</div>
